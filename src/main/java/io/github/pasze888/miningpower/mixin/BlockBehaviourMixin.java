...
import io.github.pasze888.miningpower.Config;
...
/// 该行为由 `Config#UNBREAKABLE_IF_CANNOT_HARVEST` 控制（默认启用）；关闭时不干预进度，
/// 退回原版「工具不对」的慢速破坏（不掉落）。
...
public abstract class BlockBehaviourMixin {
    ...
    private float miningpower$deny(float original, @Local(argsOnly = true) BlockState state, @Local int i) {
        if (i > 30 && Config.UNBREAKABLE_IF_CANNOT_HARVEST.getAsBoolean() && state.is(ModTags.Blocks.UNBREAKABLE_IF_CANNOT_HARVEST)) {
            return 0.0F;
        }
        return original;
    }
